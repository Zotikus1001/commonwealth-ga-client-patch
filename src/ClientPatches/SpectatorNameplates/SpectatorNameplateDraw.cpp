...
const FStringView* ResolveName(RepInfoPlayerView* pri) {
	if (pri->PlayerName.Data && pri->PlayerName.Count > 1) return &pri->PlayerName;
	if (pri->r_sOrigPlayerName.Data && pri->r_sOrigPlayerName.Count > 1) {
...
struct ActorView {
	...
	int          r_nHealthMaximum;  // ATgPawn — NOT APawn::HealthMax, which is
	                                // only ever the class default and goes
	                                // stale (see SpectatorOverlayFeed.cpp)
...
// Plate height and text scale are hot-reloaded from SpectatorNameplates.ini —
// see SpectatorNameplateSettings. Defaults live there.
...
void FillRect(CanvasView* canvas, float x, float y, float w, float h,
	...
	for (int i = 0; i < rows; ++i) {
		const float ly = y + static_cast<float>(i);
		const float start[2] = {canvas->OrgX + x,     canvas->OrgY + ly};
		const float end[2]   = {canvas->OrgX + x + w, canvas->OrgY + ly};
...
#define NP_STOP(reason) do { } while (0)
	...
	for (int i = 0; i < gri->PRIArray.Count; ++i) {
...
#ifdef GA_CLIENT_DEBUG
		// Every candidate, every report — not just the first. Correlating name
		// against world position and projected position is the only way to tell
		// a wrong pawn from a wrong projection, and a repeated name here would
		// prove a genuine double-render.
		if (report) {
			Logger::Log("clientpatch",
		...
		// somewhere plausible on screen — testing Z is not enough. Use the same
		// dot product DrawActorOverlays itself uses to decide what is in front.
		if (haveCamera) {
		...
		drawText(canvas, /*bDraw=*/0, &outXL, &outYL, font,
			cfg.scale, cfg.scale, /*bCentered=*/0, name.Data);
		...
		if (cfg.healthDisplay != 0) {
			...
			if (cfg.healthDisplay == 1) {
				...
				const float barH = static_cast<float>(cfg.barHeight) * cfg.scale;
				...
				FormatStats(stats, 64, hp, hpMax,
					static_cast<int>(pw + 0.5f), static_cast<int>(pwMax + 0.5f),
					cfg.showPowerBar);
				int statsXL = 0, statsYL = 0;
				canvas->CurX = 0.0f;
				canvas->CurY = 0.0f;
				drawText(canvas, /*bDraw=*/0, &statsXL, &statsYL, font,
					cfg.scale, cfg.scale, /*bCentered=*/0, stats);
