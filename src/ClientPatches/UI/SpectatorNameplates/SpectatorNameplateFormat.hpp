...
inline constexpr int kScalePercentMax = 400;
inline constexpr int kHeightOffsetMin = -10000;
inline constexpr int kHeightOffsetMax = 10000;
...
inline constexpr int kScaledBarHeightMax = 62;
inline constexpr int kFillRectRowMax  = 64;
inline constexpr int kPriEntriesMax   = 256;
inline constexpr int kNameCodeUnitsMax = 128;
...
// caller's default; oversized values are capped because scale also multiplies
// the per-frame health-bar drawing work.
inline float ScaleFromPercent(int percent, float fallback) {
	if (percent < kScalePercentMin) return fallback;
	return static_cast<float>(ClampInt(percent, kScalePercentMin, kScalePercentMax)) /
		100.0f;
}

inline float MultiplyScale(float base, float multiplier) {
	if (!std::isfinite(base) || base <= 0.0f) return 1.0f;
	if (!std::isfinite(multiplier) || multiplier <= 0.0f) multiplier = 1.0f;
	const float combined = base * multiplier;
	const float maximum = static_cast<float>(kScalePercentMax) / 100.0f;
	return combined > maximum ? maximum : combined;
}
...
inline float HeightOffsetFromIni(int value) {
	return static_cast<float>(ClampInt(value, kHeightOffsetMin, kHeightOffsetMax));
}
...
// Engine-owned arrays still need a hard iteration bound: a stale or corrupt
// Count must not turn a HUD frame into an unbounded walk.
inline int SafeArrayCount(int count, int maximum) {
	if (count <= 0 || maximum < count) return 0;
	return ClampInt(count, 0, kPriEntriesMax);
}

inline bool IsValidNameCount(int count, int maximum) {
	return count > 1 && count <= maximum && count <= kNameCodeUnitsMax;
}

// Filled bars are emitted as one horizontal line per row. Keep the combined
// BarHeight/ScalePercent setting within a fixed per-plate render budget.
inline float ScaledBarHeight(int height, float scale) {
	const float scaled = static_cast<float>(height) * scale;
	if (!(scaled > 0.0f)) return 0.0f;
	return scaled > static_cast<float>(kScaledBarHeightMax)
...
inline int RoundedStat(float value) {
	if (!std::isfinite(value) || value <= 0.0f) return 0;
	const float maxInt = static_cast<float>(std::numeric_limits<int>::max());
	if (value >= maxInt) return std::numeric_limits<int>::max();
